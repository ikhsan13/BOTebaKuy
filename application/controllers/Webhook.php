...
class Webhook extends CI_Controller {
	...
	function responseTextMessage($event)
	{
		...
		if($this->user['state'] < 1)
		{
			...
			if(strpos('mulai', $text) === FALSE)
			{
				...
				// send question
				$this->line_model->pushTextMessage($this->user['uid'], "Hayo coba tebak " . $question['hint']);
			}
		}
		...
		// for next state, user has his own question placed in $this->user['answer']
			...
			if($text == $this->user['answer'])
			{
				...
				$this->line_model->pushTextMessage($this->user['uid'], 'Ayo lagi! kali ini aku tantang kasih soal yang lebih sulit lagi deh! Ketik "mulai" atau klik tombol mulai di menu kapanpun Kamu siap. Kalo berani!😎');

			} else {
				if($this->user['state'] == 3)
				{
					$this->line_model->pushStickerMessage($this->user['uid'], 1, 100);
					$this->line_model->pushTextMessage($this->user['uid'], "Ow ow owww.. masih kurang tepat. Hayoo kesempatan menebak tinggal 2 kali lagi eaa");
				} else if($this->user['state'] == 2) {
					$this->line_model->pushStickerMessage($this->user['uid'], 2, 141);
					$this->line_model->pushTextMessage($this->user['uid'], "NO. Masih salah. Hayo, tinggal satu kesempatan menebak lagi. Pikirkan baik-baik!");
				} else {
					$this->line_model->pushStickerMessage($this->user['uid'], 1, 15);
	...
	function responseFollowEvent($event)
	{
		...
		$this->line_model->pushTextMessage($user['uid'], "Pada game tebak tebakan ini Kamu diminta untuk menebak kata apa yang aku maksud. Aku akan memberikan satu petunjuk yang mengarah ke jawaban yang dimaksud. Kamu punya kesempatan maksimal 3 kali untuk menebak.");
		$this->line_model->pushTextMessage($user['uid'], 'Untuk memulai silakan ketikkan perintah "mulai" atau tekan tombol mulai di menu');
	}
